  ...
  const applyErrorToData = (generatedData, error) => {
    ...
    for (let i = 0; i < numRecords; i++) {
      ...
      for (let j = 0; j < numErrors; j++) {
        ...
        switch (errorType) {
          case 'delete':
            const deleteIndex = Math.floor(Math.random() * record[field].length);
            record[field] =
            record[field].slice(0, deleteIndex) + record[field].slice(deleteIndex + 1);
            break;
          case 'add':
            const addIndex = Math.floor(Math.random() * (record[field].length + 1));
            const randomChar = alphabet.charAt(Math.floor(Math.random() * alphabet.length));
            record[field] =
            record[field].slice(0, addIndex) + randomChar + record[field].slice(addIndex);
            break;
          case 'swap':
            const swapIndex = Math.floor(Math.random() * (record[field].length - 1));
            record[field] =
